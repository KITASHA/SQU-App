...
# アプリケーション概要 
岡山アカペラサークル「SQUARE」において運営の基盤となる機能を備えたウェブアプリケーションです。下記の機能を実装または実装予定です。
...
※この操作は参加者が行うことを想定。
4. 「バンドを分ける」ボタンをクリックしてメンバーをシャッフルした上で曲ごとに必要なバンド数分バンドを生成して表示します。※ページを更新すると再生成されます。※この操作は主体者が行うことを想定。
...
- ログイン状態で「定期活動会楽譜」カードの「こちらから」をクリックするとGoogleDriveの楽譜フォルダに飛びます。※コード上は環境変数に設定したURLを読み込むようにしてURLが流出しないようにしています。※現在はテストページへのリンクを設定しています。

# 作成した背景
- サークル運営に際してX、Instagram、YouTube、LINE、Google DriveなどさまざまなSNSやツールを使い分けていて管理が面倒という課題がありました。
...
- 曲が1曲しかない場合や、参加者が1曲しかやらない場合も対応可能です。
- パートが5つ以下の曲でも対応可能です。

# 改善点
- 下記の機能を改善すれば実用化が可能です。
- 1曲につき1パートしか選べない（bassかV.P.どちらかを希望などには対応できない）。
